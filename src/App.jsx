...
import { ShoppingCart, Trash2, Plus, Minus, Calculator, FileSpreadsheet, Calendar, Edit, Save, X, Tag } from 'lucide-react';
...
function App() {
  ...
  const [menu, setMenu] = useState(() => { const saved = localStorage.getItem('spicy_menu'); return saved ? JSON.parse(saved) : [{ id: 1, name: '辣燙套餐', price: 100 }, { id: 2, name: '王子麵', price: 20 }, { id: 3, name: '牛肉片', price: 50 }, { id: 4, name: '老油條', price: 35 }]; });
  ...
  const [editMenuData, setEditMenuData] = useState({ name: '', price: '' });
  ...
  const [activePromoId, setActivePromoId] = useState(''); // 前台選擇的優惠
  ...
  const addProduct = (e) => {
    ...
    setMenu([...menu, { id: Date.now(), name: newProdName, price: Number(newProdPrice) }]);
    setNewProdName('');
    setNewProdPrice('');
  };
  const startEditMenu = (item) => { setEditingMenuId(item.id); setEditMenuData({ name: item.name, price: item.price }); };
  const saveMenuEdit = () => {
    setMenu(menu.map(m => m.id === editingMenuId ? { ...m, name: editMenuData.name, price: Number(editMenuData.price) } : m));
            ...
            <div className="grid grid-cols-4 gap-4">
              {menu.map(item => (
                <button key={item.id} onClick={() => addToCart(item)} className="bg-[#E6D2C3] border border-[#B08D79] p-5 rounded-xl shadow hover:bg-[#d8c0ad] flex flex-col justify-between h-28 font-bold text-lg active:scale-95 transition-transform">
                  ...
                  <span className="text-right text-xl text-black">${item.price}</span>
                </button>
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="bg-[#C5A087] p-5 rounded-xl border border-[#B08D79] shadow">
                ...
                <form onSubmit={addProduct} className="flex gap-2 mb-4">
                  ...
                  <input type="number" placeholder="價格" value={newProdPrice} onChange={e => setNewProdPrice(e.target.value)} className="px-3 py-2 bg-[#E6D2C3] rounded border border-[#B08D79] w-24 text-black font-bold placeholder-gray-600" />
                  ...
                  {menu.map(item => (
                    <div key={item.id} className="flex items-center justify-between bg-[#E6D2C3] p-2 rounded border border-[#B08D79]">
                      {editingMenuId === item.id ? (
                        <>
                          <input className="w-1/2 px-2 py-1 border rounded bg-white font-bold" value={editMenuData.name} onChange={e => setEditMenuData({...editMenuData, name: e.target.value})} />
                          <input type="number" className="w-1/4 px-2 py-1 border rounded bg-white font-bold" value={editMenuData.price} onChange={e => setEditMenuData({...editMenuData, price: e.target.value})} />
                        ...
                        <>
                          ...
                          <span className="font-bold text-black w-20">${item.price}</span>
                          <div className="flex gap-1">
